    ...
    if (recv.head != 0xb5){
      ...
      if (position < 3){
        ...
        if (recv.tail == 0x5b){
          ...
          /*
          char a[32];
          sprintf(a, "0x%x 0x%x 0x%x 0x%x 0x%x 0x%x END", rpi_msg.head, rpi_msg.throttle[0], rpi_msg.throttle[1], rpi_msg.throttle[2], rpi_msg.mask, rpi_msg.tail);
          Serial.println(a);
          */
        }
...
void msg_req(){
    ...
    int pravi_voltage = (int)voltage*1000;
    Wire.write(pravi_voltage);
}
